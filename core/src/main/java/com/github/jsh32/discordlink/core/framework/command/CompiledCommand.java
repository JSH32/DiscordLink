...
public class CompiledCommand {
    ...
    public CompiledCommand(Object command, Locale locale) throws CommandCompileException {
        ...
        subCommands = Arrays.stream(command.getClass().getDeclaredMethods())
                ...
                .map(m -> new CommandData(command, locale, m, false, getArguments(m), m.getAnnotation(Command.class).userOnly()))
                .collect(Collectors.toUnmodifiableSet());
